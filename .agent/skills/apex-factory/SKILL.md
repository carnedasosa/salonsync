...
Output atteso: un piano strutturato in markdown, chiaro e azionabile, pronto per essere approvato dall'utente.
```
...
Output atteso: codice componente completo, pulito, pronto per essere integrato da The Weaver.
```
...
Output atteso: codice completo, con gestione degli errori, pronto per il Quality Gate di The Critic.
```
...
Se trovi un bug significativo:
- Descrivi il problema con precisione (file, riga, natura del bug).
- Proponi o implementa la soluzione.
- Segnala all'Orchestratore che questo errore va loggato nella Knowledge Base.
...
3. Verifica che il sistema funzioni end-to-end: il frontend chiama correttamente il backend? I dati fluiscono come previsto?
4. Controlla che nessun file esistente sia stato rotto dall'integrazione.
5. Esegui il build/dev server per verificare che non ci siano errori di compilazione.

Se trovi problemi di integrazione, riportali all'Orchestratore prima di procedere.

Output atteso: conferma che il sistema integrato funziona correttamente, con eventuale lista di aggiustamenti minori effettuati.
```
